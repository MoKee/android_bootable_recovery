...
ifneq (,$(findstring extended,$(BOARD_USE_CUSTOM_RECOVERY_FONT)))
  BOARD_FONT_CHAR_MAX := 224
  FIELD_SEP := 3
else
  BOARD_FONT_CHAR_MAX := 96
  FIELD_SEP := 2
endif

BOARD_RECOVERY_CHAR_WIDTH := $(shell echo $(BOARD_USE_CUSTOM_RECOVERY_FONT) | cut -d _  -f $(FIELD_SEP) | cut -d . -f 1 | cut -d x -f 1)
BOARD_RECOVERY_CHAR_HEIGHT := $(shell echo $(BOARD_USE_CUSTOM_RECOVERY_FONT) | cut -d _  -f $(FIELD_SEP) | cut -d . -f 1 | cut -d x -f 2)

LOCAL_CFLAGS += -DBOARD_RECOVERY_CHAR_WIDTH=$(BOARD_RECOVERY_CHAR_WIDTH) -DBOARD_RECOVERY_CHAR_HEIGHT=$(BOARD_RECOVERY_CHAR_HEIGHT) -DBOARD_FONT_CHAR_MAX=$(BOARD_FONT_CHAR_MAX)
