...
void ScreenRecoveryUI::draw_menu_and_text_buffer_locked(
  ...
  if (menu_) {
    ...
    if (mokee_logo_ && back_icon_) {
      auto logo_width = gr_get_width(mokee_logo_.get());
      auto logo_height = gr_get_height(mokee_logo_.get());
      auto centered_x = ScreenWidth() / 2 - logo_width / 2;
      DrawSurface(mokee_logo_.get(), 0, 0, logo_width, logo_height, centered_x, y);
      y += logo_height;
...
bool ScreenRecoveryUI::Init(const std::string& locale) {
  ...
  mokee_logo_ = LoadBitmap("logo_image");
...
int ScreenRecoveryUI::SelectMenu(const Point& point) {
  ...
  if (menu_) {
    if (!menu_->IsMain()) {
      // Back arrow hitbox
      const static int logo_width = gr_get_width(mokee_logo_.get());
      const static int logo_height = gr_get_height(mokee_logo_.get());
